...
import { EmbedBuilder, AttachmentBuilder, GuildMember, TextChannel } from 'discord.js';
 ...
 */
export async function sendWelcomeMessage(member: GuildMember, channel: TextChannel): Promise<boolean> {
    try {
        ...
        if (process.env.FITUR_WELCOME_BANNER?.toLowerCase() === 'on') {
            try {
                ...
                if (bannerChannelId) {
                    const bannerChannel = member.guild.channels.cache.get(bannerChannelId) as TextChannel;
                    if (bannerChannel) {

...
export default {
    ...
    async execute(message: Message, args: string[], client: Client) {
        ...
        if (client.user) {
            embed.setFooter({ text: `Wonderplay Bot • Dipanggil oleh ${message.author.tag}`, iconURL: client.user.displayAvatarURL() });
        } else {
            embed.setFooter({ text: `Wonderplay Bot • Dipanggil oleh ${message.author.tag}` });
        }
